...
function extractKeywords(body, meta, maxCount) {
  ...
  const addWords = (text, weight) => {
    ...
    tokens.forEach((word) => scoreWord(word, weight));
    for (const size of [2, 3]) {
      for (let i = 0; i <= tokens.length - size; i++) {
        const phrase = tokens.slice(i, i + size).join(" ");
        if (phrase.length < 7 || phrase.length > 60) continue;
        phraseCounts.set(phrase, (phraseCounts.get(phrase) || 0) + weight * size);
      }
    }
  };
  ...
  const h1Words = new Set(tokenize(h1s.join(" ")));
  const descWords = new Set(tokenize(description));

  const phraseCandidates = [...phraseCounts.entries()]
    .filter(([phrase, score]) => {
      const tokens = phrase.split(" ");
      const structuralHit = tokens.some((word) => titleWords.has(word) || h1Words.has(word) || descWords.has(word));
      return score >= 12 || structuralHit;
    })
    .sort((a, b) => b[1] - a[1] || a[0].length - b[0].length)
    .slice(0, Math.ceil(maxCount / 2))
    .map(([phrase]) => phrase);

  const wordCandidates = [...counts.entries()]
    .filter(([word, score]) => {
      const escapedWord = word.replace(/[.*+?^()|[\]\\{}$]/g, "\\$&");
      const visibleCount = (visibleText.match(new RegExp("\\b" + escapedWord + "\\b", "g")) || []).length;
  ...
  return normalizeStringList([...phraseCandidates, ...wordCandidates]).slice(0, maxCount);
}
